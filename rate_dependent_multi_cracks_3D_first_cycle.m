...
sampling=10;
ntheta=1;
phasen=sampling*sampling*ntheta+1;

for ai=1:1
    ai
    clearvars -except phasen slideoutput inputalpha inputDS inputCG inputCK ttboundary ai sampling cracknumber ntheta
...
for i=1:sampling
    for j=1:sampling
        for k=1:ntheta
            theta((i-1)*sampling*sampling+(j-1)*sampling+k)=3*pi/4;
...
pr=zeros(phasen,1);
for i=1:phasen-1
   pr(i)=1/sampling/sampling/ntheta*((size(slideoutput,1))/bi)^(beta);
end 
temp1=sum(pr(:));
pr(phasen)=1-temp1;
