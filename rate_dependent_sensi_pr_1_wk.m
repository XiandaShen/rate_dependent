...
cstep(1)=1000;
inputpr=(0.05:0.05:0.7);

inputsize=size(inputpr,2);

vstrain_c1=zeros(inputsize,cstep(1)*2);


hstrain_c1=zeros(inputsize,cstep(1)*2);


vstress_c1=zeros(inputsize,cstep(1)*2);
...
for ai=1:inputsize
    ...
    clearvars -except phasen inputalpha inputc inputCG inputCK inputpr...
        ...
        vstress_c1 vstress_c2 hstrain_c1 hstrain_c2 inputsize
...
rmw=zeros(inputsize,1);
rde=zeros(inputsize,1);
...
for i=1:inputsize
    ...
    rmw(i)=rtw(i,cstep(1)-1);
    rde(i)=rtw(i,2*cstep(1)-1);    
end

 figure
cruves{1}=plot(inputpr*100,rmw/1000,'ok','LineWidth',1.5);
hold on
cruves{2}=plot(inputpr*100,rde/1000,'*k','LineWidth',1.5);
hold on

set(gca,'FontSize',16);
set(gcf, 'position', [200 200 500 400]);
ylabel('Energy (J/mm^3)','fontsize',16);
xlabel('\phi_c (%)','fontsize',16);
legend([cruves{1},cruves{2}],...
    'Maximum external work','Dissipated energy','Location','northwest')
