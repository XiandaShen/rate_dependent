...
inputc=(0.0005:0.0005:0.010);

inputsize=size(inputc,2);
...
R=8.314; % J/K/mol
T=297; % K
V_d_constant=16*DS*C*Omega^2/R/T*sin(alpha)^2*cos(alpha)/c^2/1000;
...
for i=1:inputsize
 ...
 figure
cruves{1}=plot(inputc*1000,rmw/1000,'ok','LineWidth',1.5);
hold on
cruves{2}=plot(inputc*1000,rde/1000,'*k','LineWidth',1.5);
hold on

set(gca,'FontSize',16);
set(gcf, 'position', [200 200 500 400]);
ylabel('Energy (J/mm^3)','fontsize',16);
xlabel('c (\mum)','fontsize',16);
legend([cruves{1},cruves{2}],...
    'Maximum external work','Dissipated energy','Location','west')
