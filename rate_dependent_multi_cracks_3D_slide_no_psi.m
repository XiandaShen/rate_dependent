...
for i=1:psampling
    for j=1:sampling
        for k=1:ntheta
            theta((i-1)*psampling+(j-1)*ntheta+k)=pi*1/18;
            psi((i-1)*psampling+(j-1)*ntheta+k)=psio(j);
            phi((i-1)*psampling+(j-1)*ntheta+k)=pi*1/9;
        end
    end
end
psi(phasen)=0;
...
pr=zeros(phasen,1);
for i=1:phasen-1
   pr(i)=1/sampling/ntheta/psampling*((size(slideoutput,1))/bi)^(beta);
end 
temp1=sum(pr(:));
pr(phasen)=1-temp1;
...
cruves{2}=plot(hstrain(1:cstep_ll(2),2)+bc{2}(1,3)-bc{1}(1,3)-hstrain(1,2),vstress(1:cstep_ll(2),2)-vstress(1,2),'-r','LineWidth',2);
hold on
cruves{3}=plot(hstrain(1:cstep_ll(3),3)+bc{3}(1,3)-bc{1}(1,3)-hstrain(1,3)+0.00002,vstress(1:cstep_ll(3),3)-vstress(1,3),'-b','LineWidth',2);
hold on
cruves{4}=plot(bc{1}(:,3)-bc{1}(1,3),bc{1}(:,5),'--k','LineWidth',2);
cruves{5}=plot(bc{2}(:,3)-bc{1}(1,3),bc{2}(:,5),'--r','LineWidth',2);
cruves{6}=plot(bc{3}(:,3)-bc{1}(1,3),bc{3}(:,5),'--b','LineWidth',2);
